...
public class ModifyArticleHandler implements CommandHandler {
	...
	private String processForm(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try{
			...
			//있으면 modifyRequest에 담아서 수정 화면을 보여준다.
			ModifyRequest modReq = new ModifyRequest(authUser.getUserId(), no, articleData.getArticle().getTitle(), articleData.getContent());
	...
	private String processSubmit(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		ModifyRequest modReq = new ModifyRequest(authUser.getUserId(), no, req.getParameter("title"), req.getParameter("content"));
		req.setAttribute("modReq", modReq);
